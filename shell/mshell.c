	...
	for(int i = 0; (line -> pipelines)[i] != NULL; ++i){ // for each pipeline
		...
		for(int u = 0; cur_pipeline[u]; ++u){
			...
			if(child_pid > 0){
				...
				unblock_sigchld();// chyba dodac, przemysl!! 
				if(u == 0){
					if(!cur_pipeline[u+1]){
						close(fd[1]);
						close(fd[0]);
...
line* prepare_line(int length, char* buffer){
	char* currentline;
	if(length == 0)
			exit(1);
...
void buffer_copy(char* tab1, char* tab2, int len1){
	if(tab1 == NULL || len1 == 0)
		return;
...
void read_and_order_executing(bool terminal_mode){
	char* buffer = (char*) calloc(2 * MAX_LINE_LENGTH + 2, sizeof(char));//buffer xD
	...
	while(true){
		...
		int length = -1;
		do{
			length = read(0, buffer+first_part_length, 
		...
		if(length == 0){
			realloc(buffer, ((2 * MAX_LINE_LENGTH+2)*sizeof(char)));
			exit(0);
		}
		...
		for(int i = 0; i < length; ++i){	
			if(buffer[i] == '\n' || buffer[i] == 0){
				...
				line* line = prepare_line(i - prev, buffer + prev);
				execute_line(line);	
				prev = i+1;
			}
		}
		...
		if(prev == 0 && length == MAX_LINE_LENGTH+1){
			...
			first_part_length = 0;
			clear_buffer(buffer, 2*MAX_LINE_LENGTH+2);
			continue;
		}

		if(prev != MAX_LINE_LENGTH+1){
			
			char* new_buffer = (char*) calloc(2 * MAX_LINE_LENGTH+2, sizeof(char));
			first_part_length = length - prev;
			
			buffer_copy(buffer+prev, new_buffer, first_part_length);
			realloc(buffer, ((2 * MAX_LINE_LENGTH+2)*sizeof(char)));
			buffer = new_buffer;
			clear_buffer(buffer+first_part_length+1, 
